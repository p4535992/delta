...
public class DocumentSearchServiceImpl extends AbstractSearchServiceImpl implements DocumentSearchService {
    ...
    @Override
    public List<NodeRef> searchOpenSeriesByDocType(QName docType) {
        String query = SearchUtil.joinQueryPartsAnd(
                    SearchUtil.generateTypeQuery(SeriesModel.Types.SERIES),
                    SearchUtil.generatePropertyExactQuery(SeriesModel.Props.DOC_TYPE, docType.toString(), true),
                    SearchUtil.generatePropertyExactQuery(SeriesModel.Props.STATUS, DocListUnitStatus.OPEN.getValueName(), false)
                );
        return searchNodes(query, false, "openSeriesByDocType");
    }
    ...
    private List<Document> searchDocumentsAndOrCases(String query, String searchString, boolean includeCaseTitles) {
        ...
        try {
            ...
            if (includeCaseTitles) {
                query = joinQueryPartsOr(Arrays.asList(query, getCaseByTitleQuery(searchString), getMaaisCaseQuery(searchString, false)));
    ...
    @Override
    public NodeRef getMaaisCase(String caseNumber) {
        List<NodeRef> searchNodes = searchNodes(getMaaisCaseQuery(caseNumber, true), false, "getMaaisCaseByNumber");
    ...
    private String getMaaisCaseQuery(String searchValue, boolean caseNumberOnly) {
        List<String> queryParts = new ArrayList<String>();
        queryParts.add(generateTypeQuery(MaaisModel.Types.MAAIS_CASE));
        if (caseNumberOnly) {
            queryParts.add(SearchUtil.generateStringExactQuery(searchValue, MaaisModel.Props.CASE_NUMBER));
        } else {
            queryParts.add(generateStringWordsWildcardQuery(searchValue, MaaisModel.Props.CASE_NUMBER, MaaisModel.Props.CASE_RELATED_PERSON, MaaisModel.Props.LAND_NAME,
                    MaaisModel.Props.LAND_NUMBER));
        }
        return joinQueryPartsAnd(queryParts, false);
    }
    ...
    @Override
    public List<Document> searchDocumentsByAspect(QName aspect) {
        List<String> queryParts = new ArrayList<String>();
        long startTime = System.currentTimeMillis();
        queryParts.add(SearchUtil.generateTypeQuery(DocumentCommonModel.Types.DOCUMENT));
        queryParts.add(SearchUtil.generateAspectQuery(aspect));
        String query = SearchUtil.joinQueryPartsAnd(queryParts);
        List<Document> result = searchDocumentsImpl(query, false, /* queryName */"documentsByAspect");
        if (log.isDebugEnabled()) {
            log.debug("Documents by aspect search total time " + (System.currentTimeMillis() - startTime) + " ms, query: " + query);
        }
        return result;
    }
    ...
    private List<ResultSet> doSearches(String query, boolean limited, int limit, String queryName, Collection<StoreRef> storeRefs) {
        SearchParameters sp = buildSearchParameters(query, limited);
        if (limit > 0) {
            sp.setLimit(limit);
        }
