...
public class GoProDocumentsMapper {
    private static org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory.getLog(GoProDocumentsMapper.class);
    ...
    protected Map<String, Mapping> loadMetadataMappings(File mappingsFile) throws Exception {
        ...
        for (Object propValuesElement: root.elements("propValues")) {        
	        if (propValuesElement != null) {
	        	String propValuesTo = ((Element)propValuesElement).attributeValue("to");
	            if ("accessRestriction".equals(propValuesTo)) {
	            	fillPropValues(mappings, "accessRestrictionValues", (Element)propValuesElement);
	            } else if ("docStatus".equals(propValuesTo)) {
	            	fillPropValues(mappings, "docStatusValues", (Element)propValuesElement);
	            } else if ("storageType".equals(propValuesTo)) {
	            	fillPropValues(mappings, "storageTypeValues", (Element)propValuesElement);
	            }
	        }
        }
    ...
    private void fillPropValues(Map<String, Mapping> mappings, String proValuesKey, Element element) {
    	
    	Mapping propValuesMapping = new Mapping(null, null, null);
        for (Object o : element.elements("value")) {
            Element el = (Element) o;
            String from = el.attributeValue("from");
            Assert.notNull(from);
            String to = el.attributeValue("to");
            Assert.notNull(to);
            propValuesMapping.add(new PropMapping(from, to, null, null));
        }
        mappings.put(proValuesKey, propValuesMapping);
    }
