                    ...
                    NodeServicePolicies.OnDeleteAssociationPolicy
{
    ...
    private boolean ignoreMissingAspects = true;
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef)
    {
        ...
        if (! storesToIgnore.contains(tenantService.getBaseName(childRef.getStoreRef()).toString()))
        {
            ...
            event = getAspectIntegrityEvent(childRef);
            save(event);
    ...
    public void onRemoveAspect(NodeRef nodeRef, QName aspectTypeQName)
    {
        if (! storesToIgnore.contains(tenantService.getBaseName(nodeRef.getStoreRef()).toString()))
        {
            IntegrityEvent event = null;
            // check mandatory aspects
            event = getAspectIntegrityEvent(nodeRef);
            save(event);
        }  
    }

    private IntegrityEvent getAspectIntegrityEvent(NodeRef nodeRef) {
        if (ignoreMissingAspects) {
            return getDummyIntegrityEvent(nodeRef);
        } else {
            return new AspectsIntegrityEvent(nodeService, dictionaryService, nodeRef);
        }
    }

    private AbstractIntegrityEvent getDummyIntegrityEvent(NodeRef nodeRef) {
        return new AbstractIntegrityEvent(nodeService, dictionaryService, nodeRef, null, null){

            @Override
            public void checkIntegrity(List<IntegrityRecord> paramList) {
