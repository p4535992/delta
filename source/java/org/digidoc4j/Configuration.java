 ...
 * </p>
 * <p/>
 * You can specify the configuration mode, either {@link Mode#TEST} or {@link Mode#PROD}
 * configuration. Default is {@link Mode#PROD}.
 * <p/>
 * <p>
 ...
 * the default mode to {@link Mode#TEST} mode
 * <p/>
...
public class Configuration implements Serializable {
  ...
  private void setJDigiDocConfigurationValue(String key, String defaultValue) {
    logger.trace("setJDigiDocConfigurationValue(): Key: [" + key + "] ==> defaultValue: ["+defaultValue+"]");
   ...
   * Default value is false for {@link Mode#PROD} and false for {@link Mode#TEST}
   *
  ...
  public long getMaxDataFileCachedInMB() {
    ...
    logger.debug("getMaxDataFileCachedInMB() Maximum datafile cached in MB: " + maxDataFileCached);

    if (maxDataFileCached == null){
      logger.debug("getMaxDataFileCachedInMB(): DIGIDOC_MAX_DATAFILE_CACHED is NULL! Returning '-1'");
  ...
  private String defaultIfNull(String configParameter, String defaultValue) {
    logger.debug("defaultIfNull(): Parameter: " + configParameter + "; defaultValue: " + defaultValue);
    if (configurationFromFile == null){
      logger.debug("defaultIfNull(): configurationFromFile is NULL! Returning defaultValue...");
      return defaultValue;
    }
    Object value = configurationFromFile.get(configParameter);
    if (value != null) {
      logger.debug("defaultIfNull() value found! Checking if it is allowed...");
    ...
    logger.debug("defaultIfNull(): configuredValue: " + configuredValue);
  ...
  private boolean valueIsAllowed(String configParameter, String value) {
    logger.debug("valueIsAllowed(): Parameter: " + configParameter + ", value: " + value);
