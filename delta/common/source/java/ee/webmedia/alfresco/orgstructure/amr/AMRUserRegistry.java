...
/**
 * A {@link UserRegistry} implementation with the ability to query Alfresco-like
 * descriptions of users and groups from a SIM "Ametnikeregister".
 * 
 * @author Ats Uiboupin
 */
public class AMRUserRegistry implements UserRegistry, ActivateableBean {
	...
	public Iterator<NodeDescription> getPersons(Date modifiedSince) {
		...
		ArrayList<NodeDescription> persons = new ArrayList<NodeDescription>(
				ametnikArray.length);
		...
		if (isRestrictedDelta) {
			// avoid retrieving restricted delta users if not in restricted
			// delta
			restrictedDeltaUsers = Arrays.asList(rsService
					.getIsikukoodByAsutusIdAndHasRsLubaRequest());
		}
		for (AmetnikExt ametnik : ametnikArray) {
			if (checkRestrictedDeltaUsers(isRestrictedDelta,
					restrictedDeltaUsers, ametnik)) {
				continue;
			}
			NodeDescription person = mergePersonDescription(ametnik);
			person.setLastModified(new Date());// actually should be when
												// modified in remote system
			persons.add(person);
			if (log.isDebugEnabled()) {
				log.debug("firstName=" + ametnik.getEesnimi() + "; lastName="
						+ ametnik.getPerekonnanimi() + "; id="
						+ ametnik.getIsikukood() + "; unitId="
						+ ametnik.getYksusId() + "; jobTitle="
						+ ametnik.getAmetikoht() + "; phone="
						+ ametnik.getKontakttelefon() + "; email="
						+ ametnik.getEmail());
			}
		}
	...
	private boolean checkRestrictedDeltaUsers(boolean isRestrictedDelta,
			List<String> restrictedDeltaUsers, AmetnikExt ametnik) {
		return isRestrictedDelta
				&& !restrictedDeltaUsers.contains(ametnik.getIsikukood());
	}
	...
	public Iterator<NodeDescription> getPersonByIdCode(String idCode) {
		...
		// TODO merge logic from AMRSimpleAuthenticationImpl to here
		// (RsAccessStatusBean and other...)
		if (rsService.isRestrictedDelta()
				&& !rsService.hasRsLubaByIsikukood(idCode)) {
			return Collections.<NodeDescription> emptyList().iterator();
		}
		return Collections.singleton(mergePersonDescription(ametnik))
				.iterator();
	}
	...
	public Iterator<NodeDescription> getGroups(Date modifiedSince) {
		...
		// List<OrganizationStructure> orgStructures = new
		// ArrayList<OrganizationStructure>(yksusArray.length);
		...
		for (YksusExt yksus : yksusArray) {
			...
			List<String> yksusRada = UserUtil
					.formatYksusRadaToOrganizationPath(yksus.getYksusRada());
			...
			for (String yksusObj : yksusRada) {
				...
				if (group == null) {
					...
					group.getProperties().put(ContentModel.PROP_AUTHORITY_NAME,
							gid);
					...
					log.debug("Found duplicate group name (" + gid
							+ "). Skip...");
				}
			}
		}
	 ...
	 *         (merged with propertis that person with the same userName has -
	 *         if such person exists)
	 */
	private NodeDescription mergePersonDescription(AmetnikExt ametnik) {
		...
		Map<QName, Serializable> existingProperties = userService
				.getUserProperties(ametnik.getIsikukood());
	...
	/**
	 * Reads properties from <code>AmetnikExt</code> object and puts them into
	 * <code>properties</code>
	 * 
	 * @param ametnik
	 * @param properties
	 */
	public void fillPropertiesFromAmetnik(AmetnikExt ametnik,
			Map<QName, Serializable> properties) {
		String email = ametnik.getEmail();

		// This is actually implemented in ChainingUserRegistrySynchronizer and
		// the correct thing to do would be to
		// switch from AMRSimpleAuthenticationImpl to
		// SimpleUpdatingAuthenticationComponentImpl - this has been the idea
		// all along, but needs testing
		...
		properties
				.put(ContentModel.PROP_TELEPHONE, ametnik.getKontakttelefon());
		properties.put(ContentModel.PROP_JOBTITLE, ametnik.getAmetikoht());
		properties.put(ContentModel.PROP_ORGANIZATION_PATH,
				(ArrayList<String>) UserUtil
						.formatYksusRadaToOrganizationPath(ametnik
								.getYksusRada()));
		Aadress aadress = ametnik.getAadress();
		if (aadress != null) {
			properties.put(ContentModel.PROP_COUNTY, aadress.getMaakond());
			properties.put(ContentModel.PROP_MUNICIPALITY,
					aadress.getOmavalitsus());
			properties
					.put(ContentModel.PROP_VILLAGE, aadress.getAsustusYksus());
			properties.put(ContentModel.PROP_STREET_HOUSE,
					aadress.getKohanimi());
			properties
					.put(ContentModel.PROP_POSTAL_CODE, aadress.getSihtkood());
		}
		properties.put(ContentModel.PROP_SERVICE_RANK,
				ametnik.getTeenistusaste());
	}

	// START: getters / setters
	/**
	 * Indicates whether this bean is active. I.e. should this part of the
	 * subsystem be used?
	 * 
	 * @param active
