...
public class FullIndexRecoveryComponent extends AbstractReindexComponent
{
    ...
    private void performFullRecovery()
    {
        ...
        while(true)
        {
        	logger.debug("[SMIT]: get list of transactions.");
        	...
        	logger.debug("[SMIT]: get list of transactions - found: " + nextTxns.size());
            ...
            while (txnIterator.hasNext())
            {
                ...
                logger.debug("[SMIT]: transaction id: [" + txnId + "]");
                ...
                if (stopOnError)
                {
                	logger.debug("[SMIT]: transaction id: [" + txnId + "] - stopOnError true - start");
                    reindexTransaction(txnId);
                    logger.debug("[SMIT]: transaction id: [" + txnId + "] - stopOnError true - end");
                }
                else
                {
                    ...
                    if (!txnIterator.hasNext() || txnIdBuffer.size() >= maxTransactionsPerLuceneCommit)
                    {
                        try
                        {
                        	logger.debug("[SMIT]: transaction id: [" + txnId + "] - reindexTransactionAsynchronously start");
                            reindexTransactionAsynchronously(txnIdBuffer);
                            logger.debug("[SMIT]: transaction id: [" + txnId + "] - reindexTransactionAsynchronously end");
                        }
                        catch (Throwable e)
                        {
                ...
                logger.debug("[SMIT]: prcessed Count: [" + processedCount + "]");
            ...
            logger.debug("[SMIT]: waitForAsynchronousReindexing - start");
            waitForAsynchronousReindexing();
            logger.debug("[SMIT]: waitForAsynchronousReindexing - end");
            // have we finished?
            if (nextTxns.size() == 0)
            {
                // there are no more
    ...
    public void reindexTransaction(final long txnId)
    {
        ...
        RetryingTransactionCallback<Object> reindexWork = new RetryingTransactionCallback<Object>()
        {
            ...
            {
            	logger.debug("transakstiooni [" + txnId + "] nodede otsing");
                // get the node references pertinent to the transaction
                List<NodeRef> nodeRefs = nodeDaoService.getTxnChanges(txnId);
                
                logger.debug("transakstiooni [" + txnId + "] noded leitud - " + nodeRefs.size());
                
                // reindex each node
                for (NodeRef nodeRef : nodeRefs)
                {
                	logger.debug("transakstiooni [" + txnId + "] Leiame node staatuse - [" + nodeRef.getId() + "]");
                	
                    Status nodeStatus = nodeService.getNodeStatus(nodeRef);
                    
                    if (nodeStatus == null)
                    {
                    	logger.debug("transakstiooni [" + txnId + "] node [" + nodeRef.getId() + "] nodeStatus objekt on null");
                        // it's not there any more
                        continue;
                    }
                    if (nodeStatus.isDeleted())                                 // node deleted
                    {
                    	logger.debug("transakstiooni [" + txnId + "] node [" + nodeRef.getId() + "] staatus on kustutatud");
                    ...
                    {
                    	logger.debug("transakstiooni [" + txnId + "] node [" + nodeRef.getId() + "] staatus on update");
                        // reindex
