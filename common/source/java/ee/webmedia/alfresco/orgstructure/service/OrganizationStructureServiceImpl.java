...
public class OrganizationStructureServiceImpl implements OrganizationStructureService {
    ...
    public int updateOrganisationStructureBasedGroups() {
        ...
        Map<Integer, OrganizationStructure> orgStructById = new HashMap<Integer, OrganizationStructure>(groupCount);
        ...
        for (OrganizationStructure os : allOrganizationStructures) {
            ...
            if (!isGeneratedGroupAuthority) {
            	log.info("Group not found. Creating [nr "+i+" of "+groupCount+"] ("+groupName+") from AMR...");
            ...
            OUTER: for (Map<QName, Serializable> props : users) {
                    ...
                    for (String op : orgPath) {
                        if (StringUtils.equals(op, groupName)) {
                            if (!isAlreadyGroupMember) {
								...
								log.trace("Group [nr "+i+" of "+groupCount+"] ("+groupName+"): Adding user ("+username+")...");
                            }
                ...
                if (orgId == null) {
                	log.trace("Group [nr "+i+" of "+groupCount+"] ("+groupName+"): user ("+username+"): Org ID is null. Continue..");
                    continue;
                }
    ...
    @Override
    public int removeOrganisationStructureBasedGroupsSim() {
        NodeRef zone = authorityService.getOrCreateZone(STRUCT_UNIT_BASED);
        String structUnitZoneName = (String) nodeService.getProperty(zone, ContentModel.PROP_NAME);
    ...
    @Override
    public void removeOrganisationStructureBasedGroupsSim(ActionEvent event) {
        AuthenticationUtil.runAs(new RunAsWork<Integer>() {
            @Override
            public Integer doWork() throws Exception {
                return removeOrganisationStructureBasedGroupsSim();
            }
        }, AuthenticationUtil.getSystemUserName());
