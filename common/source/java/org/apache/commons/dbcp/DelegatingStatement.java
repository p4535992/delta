...
import ee.webmedia.alfresco.utils.StatisticsCollectingStatement;
...
public class DelegatingStatement extends AbandonedTrace implements Statement {
    /** My delegate. */
    protected StatisticsCollectingStatement _stmt = null;
    ...
    public DelegatingStatement(DelegatingConnection c, Statement s) {
        super(c);
        _stmt = new StatisticsCollectingStatement(s);
        _conn = c;
    }
    ...
    protected void activate() throws SQLException {
        if(_stmt.getDelegate() instanceof DelegatingStatement) {
            ((DelegatingStatement)_stmt.getDelegate()).activate();
        }
    }

    protected void passivate() throws SQLException {
        if(_stmt.getDelegate() instanceof DelegatingStatement) {
            ((DelegatingStatement)_stmt.getDelegate()).passivate();
        }
    }
    ...
    return _stmt.getDelegate().toString();
    }
...
/* JDBC_4_ANT_KEY_BEGIN */
    ...
    public <T> T unwrap(Class<T> iface) throws SQLException {
        ...
        } else if (iface.isAssignableFrom(_stmt.getDelegate().getClass())) {
            return iface.cast(_stmt.getDelegate());
        } else {
