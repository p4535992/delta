...
public class JspStateManagerImpl
     ...
     * Without calling this hack, the next request by user would be cought in the check that view state id is not the latest available.
     * 
     * Based on ViewSequenceUtils.nextViewSequence.
     */
    @SuppressWarnings("unchecked")
    public static void ignoreCurrentViewSequenceHack() {
        ...
        // are handled at the same time for the session
        {
            ...
            if (sequence == null || sequence.intValue() == Integer.MAX_VALUE || sequence.intValue() < 2) {
