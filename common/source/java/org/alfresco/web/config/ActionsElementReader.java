...
public class ActionsElementReader implements ConfigElementReader
{
    ...
    public static final String ATTRIBUTE_SHARED_RESOURCE = "shared-resource";
    ...
    public ConfigElement parse(Element element)
    {
        ...
        if (element != null)
        {
            ...
            while (actionGroupItr.hasNext())
            {
                ...
                String sharedResource = groupElement.attributeValue(ATTRIBUTE_SHARED_RESOURCE);
                if (sharedResource != null) {
                    String className = sharedResource.trim();
                    try {
                        Class<EvaluatorSharedResource<Serializable>> clazz = (Class<EvaluatorSharedResource<Serializable>>) Class.forName(className);
                        actionGroup.setSharedResources(clazz);
                    } catch (Throwable err) {
                        throw new ConfigException("Unable to create shared resource class (classname: " + className + ")");
                    }
                }
