...
public class NavigatorPluginBean implements IContextListener,Serializable
{
    ...
    * 'noderef' parameter. retrieveChildren is for future development (cache + state)
    * Doesn't hold any caches that retrieveChildren does. Obsolete when we don't need "areas" (e.g. companyhome). 
    */
   public void retrieveNodeChildren()
   {
      ...
      try
      {
         ...
         if (logger.isDebugEnabled())
            logger.debug("retrieveChildren: nodeRef = " + nodeRefStr);
   ...
   /**
    * Retrieves the number on children for current node
    */
   public void childrenCount() throws IOException
   {
       FacesContext context = FacesContext.getCurrentInstance();
       ResponseWriter out = context.getResponseWriter();
       int childrenCount = 0;
       ...
       try
       {
          ...
          Map params = context.getExternalContext().getRequestParameterMap();
          String nodeRef = (String) params.get("nodeRef");
          
          List<ChildAssociationRef> childRefs = this.getNodeService().getChildAssocs(new NodeRef(nodeRef), 
                  ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL);
          ...
          for (ChildAssociationRef ref: childRefs)
          {
             ...
             if (isAddableChild(childNodeRef))
             {
                 childrenCount++;
             }
          }
          
          StringBuilder xml = new StringBuilder("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?><childrencount>");
          xml.append(childrenCount);
          xml.append("</childrencount>");
   ...
   {
       return createTreeNode(nodeRef, false);
   }
    ...
    * @param childrenCount 
    */
   protected TreeNode createTreeNode(NodeRef nodeRef, boolean childrenCount)
   {
       int count = 0;
       if(childrenCount) {
           count = this.getNodeChildrenCount(nodeRef);
       }
       
       TreeNode node = new TreeNode(nodeRef.toString(), 
               Repository.getNameForNode(this.getInternalNodeService(), nodeRef),
               (String)this.getInternalNodeService().getProperty(nodeRef, ApplicationModel.PROP_ICON), count);
   ...
   private int getNodeChildrenCount(NodeRef nodeRef) {
       return Repository.getServiceRegistry(FacesContext.getCurrentInstance()).getFileFolderService().listFolders(nodeRef).size();
   }
