...
import ee.webmedia.alfresco.common.search.SearchExtensionService;
import ee.webmedia.alfresco.common.search.SearchQueryBuilder;
...
public class SearchContext implements Serializable
{
   ...
   {
      SearchExtensionService searchExtensionService = (SearchExtensionService) FacesContextUtils.getRequiredWebApplicationContext(
            FacesContext.getCurrentInstance()).getBean(SearchExtensionService.BEAN_NAME);
      SearchQueryBuilder builder = searchExtensionService.getSearchQueryBuilder(this);
      ...
      if (text.length() != 0 && text.length() >= minimum)
      {
         if (text.indexOf(' ') == -1 && text.charAt(0) != '"')
         {
            ...
            if (text.length() != 0)
            {
               ...
               // If * wildcard hasn't been added, append it to text
               if(!text.contains(STR_OP_WILDCARD))
               {
                  text += STR_OP_WILDCARD;
               }
               ...
               builder.processSingleWordSearch(text, operatorNOT);
            }
         }
         else
         {
            ...
            if (text.charAt(0) == '"' && text.charAt(text.length() - 1) == '"')
            {
               ...
               builder.processMultipleWordSearchAsSingleQuotedPhrase(quotedSafeText);
            }
            else
            {
               ...
               builder.preProcessMultipleWordSearchAsIndividualSearchTerms();
               ...
               for (int i=0; i<tokenCount; i++)
               {
                  ...
                  if (term.length() != 0)
                  {
                     ...
                     builder.processMultipleWordSearchAsIndividualSearchTerms(term, operatorAND, operatorNOT);
               ...
               nameAttrBuf.append(')');
               additionalAttrsBuf.append(')');
               builder.postProcessMultipleWordSearchAsIndividualSearchTerms();
            }
         }
    ...
    */
   public static void processSearchAttribute(QName qname, String value, StringBuilder buf, boolean andOp, boolean notOp)
   {
      if (andOp) buf.append('+');
