    ...
    implements ImporterService
{
        ...
        implements Importer
    {
        ...
        public NodeRef importNode(ImportNode context)
        {
            ...
            findAndImportContent(context, nodeRef);
                ...
                if (importedRef.value != null)
                {
                    if (importedRef.value instanceof Collection)
                    {
                        ...
                        for (String unresolvedRef : unresolvedRefs)
                        {
                            ...
                            {
                                NodeRef nodeRef = resolveImportedNodeRef(importedRef.getContext().getNodeRef(), unresolvedRef);
                    ...
                    {
                        refProperty = resolveImportedNodeRef(importedRef.getContext().getNodeRef(), (String)importedRef.value);
                        // TODO: Provide a better mechanism for invalid references? e.g. report warning
                    }
                }
                
                // Set node reference on source node
                Set<QName> disabledBehaviours = getDisabledBehaviours(importedRef.getContext());
                try
                {
                    for (QName disabledBehaviour: disabledBehaviours)
                    {
                        behaviourFilter.disableBehaviour(importedRef.getContext().getNodeRef(), disabledBehaviour);
                    }
                    nodeService.setProperty(importedRef.getContext().getNodeRef(), importedRef.property, refProperty);
                    if (progress != null)
                    {
                        progress.propertySet(importedRef.getContext().getNodeRef(), importedRef.property, refProperty);
                    }
                }
                finally
                {
                    behaviourFilter.enableBehaviours(importedRef.getContext().getNodeRef());
                }
            }
        ...
        protected class UpdateExistingNodeImporterStrategy implements NodeImporterStrategy
        {
            ...
            public NodeRef importNode(ImportNode node)
            {
                // replace existing node, if node to import has a UUID and an existing node of the same
                // uuid already exists
                NodeRef existingNodeRef = getExistingNodeRef(node);
