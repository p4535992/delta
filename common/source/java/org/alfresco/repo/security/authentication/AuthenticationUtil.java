...
public class AuthenticationUtil implements InitializingBean
{
    ...
    public static <R> R runAs(RunAsWork<R> runAsWork, String uid)
    {
        ...
        boolean isSystemUser = getSystemUserName().equals(uid);
        // Save overlay info
        if (isSystemUser) {
            threadLocalOriginalFullAuthenticationForSystemOverlay.set(originalRunAsAuthentication);
            threadLocalOriginalRunAsAuthenticationForSystemOverlay.set(originalRunAsAuthentication);
        }
        ...
        finally
        {
            ...
            // Reset the overlay info
            if (isSystemUser) {
                threadLocalOriginalFullAuthenticationForSystemOverlay.set(null);
                threadLocalOriginalRunAsAuthenticationForSystemOverlay.set(null);
            }
        }
    }

    public static String getOriginalFullAuthenticationForSystemOverlay() {
        Authentication authentication = threadLocalOriginalFullAuthenticationForSystemOverlay.get();
        return authentication == null ? null : getUserName(authentication);
    }

    public static String getOriginalRunAsAuthenticationForSystemOverlay() {
        Authentication authentication = threadLocalOriginalRunAsAuthenticationForSystemOverlay.get();
        return authentication == null ? null : getUserName(authentication);
    }

    private static ThreadLocal<Authentication> threadLocalOriginalFullAuthenticationForSystemOverlay = new ThreadLocal<Authentication>();
    private static ThreadLocal<Authentication> threadLocalOriginalRunAsAuthenticationForSystemOverlay = new ThreadLocal<Authentication>();
