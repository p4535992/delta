...
public class LDAPUserRegistry implements UserRegistry, InitializingBean, ActivateableBean
{
    ...
    private String personUsernameQuery = "(&(objectclass=inetOrgPerson)(uid={0}))";

    private String personIdCodeQuery = "(&(objectclass=inetOrgPerson)(uid={0}))";
    ...
    public Iterator<NodeDescription> getPersonByUsername(String username) {
        return new PersonIterator(username, false);
    }

    @Override
    public Iterator<NodeDescription> getPersonByIdCode(String idCode) {
        return new PersonIterator(idCode, true);
    }
    ...
    public class PersonIterator implements Iterator<NodeDescription>
    {
        ...
        private String idCodeOrUsername;
        private String idCodeOrUsernameQuery;

        public PersonIterator(String idCodeOrUsername, boolean idCode)
        {
            this.idCodeOrUsername = idCodeOrUsername;
            if (idCode) {
                idCodeOrUsernameQuery = personIdCodeQuery;
            } else {
                idCodeOrUsernameQuery = personUsernameQuery;
            }
            init();
        }
        ...
        private NodeDescription fetchNext() throws NamingException
        {
            ...
            do
            {
                ...
                if (readyForNextPage)
                {
                    if (this.idCodeOrUsername != null)
                    {
                        this.searchResults = this.ctx.search(LDAPUserRegistry.this.userSearchBase,
                                this.idCodeOrUsernameQuery, new Object[] { this.idCodeOrUsername }, this.userSearchCtls);
                    }
                    else if (this.modifiedSince == null)
                    {
                        this.searchResults = this.ctx.search(LDAPUserRegistry.this.userSearchBase,
