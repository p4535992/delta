...
public class RepositoryDescriptorDAOImpl implements DescriptorDAO
{
    ...
    private ImporterBootstrap getSystemBootstrap() {
        if (systemBootstrap == null) {
            systemBootstrap = BeanHelper.getSpringBean(ImporterBootstrap.class, "systemBootstrap");
        }
    ...
    private NodeRef getDescriptorNodeRef(final boolean create)
    {
        ...
        StoreRef storeRef = getSystemBootstrap().getStoreRef();
        final Properties systemProperties = getSystemBootstrap().getConfiguration();
