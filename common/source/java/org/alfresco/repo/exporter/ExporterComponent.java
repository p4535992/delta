...
{
    private static Log log = LogFactory.getLog(ExporterComponent.class);
    // Supporting services
    ...
    @Override
    public org.w3c.dom.Node exportViewNode(ExporterCrawlerParameters parameters) {
        ViewXMLExporter exporter = null;
        SAXContentHandler saxContentHandler = new SAXContentHandler();
        try
        {
            exporter = new ViewXMLExporter(namespaceService, nodeService, searchService, dictionaryService, permissionService, saxContentHandler);
            exporter.setReferenceType(parameters.getReferenceType());
        }
        catch (Exception e)        
        {
            throw new ExporterException("Failed to create DOM Node Writer for export", e);            
        }
        // export        
        exportView(exporter, parameters, null);
        try {
            return (new DOMWriter()).write(saxContentHandler.getDocument());
        }
        catch (DocumentException e){
            log.debug("Errro converting document node: " + saxContentHandler.getDocument());
    ...
    private class DefaultCrawler implements ExporterCrawler
    {
        ...
        List<NodeRef> excludedNodeRefs = null;
        List<QName> excludedProperties = null;
        List<QName> excludedAssocTypes = null;
        ...
        public void export(ExporterCrawlerParameters parameters, Exporter exporter)
        {
            ...
            if(parameters instanceof ExcludingExporterCrawlerParameters){
                excludedNodeRefs = ((ExcludingExporterCrawlerParameters) parameters).getExcludeNodeRefs();
                excludedProperties = ((ExcludingExporterCrawlerParameters) parameters).getExcludedProperties();
                excludedAssocTypes = ((ExcludingExporterCrawlerParameters) parameters).getExcludedAssocTypes();
            } else {
                excludedNodeRefs = null;
                excludedProperties = null;
                excludedAssocTypes = null;
            }                
            ...
            while (context.canRetrieve())
            {
                ...
                else if (parameters.isCrawlChildNodes())
                {
                    ...
                    {
                        if (excludedAssocTypes != null && !isExcludedAssocType(childAssoc)) {
                            walkStartNamespaces(parameters, exporter);
        ...
        private void walkNode(NodeRef nodeRef, ExporterCrawlerParameters parameters, Exporter exporter, boolean exportAsRef)
        {
            ...
            for (QName property : properties.keySet())
            {
                ...
                if(excludedProperties != null && isExcludedProperty(property)){
                    continue;
                }
        ...
        public boolean isExcludedAssocType(ChildAssociationRef childAssoc) {
            return excludedAssocTypes.contains(childAssoc.getTypeQName());
        }

        public boolean isExcludedProperty(QName property) {
            return excludedProperties.contains(property);
        }
        
        private boolean isExcludedNodeRef(NodeRef nodeRef) {
            if (excludedNodeRefs != null) {
                return excludedNodeRefs.contains(nodeRef);
            }
            return false;
        }
        ...
        private void walkNodeSecondaryLinks(NodeRef nodeRef, ExporterCrawlerParameters parameters, Exporter exporter)
        {
            ...
            for (ChildAssociationRef childAssoc : childAssocs)
            {
                ...
                if (isExcludedNodeRef(childAssoc.getChildRef())) {
                    return;
                }                  
        ...
        private void walkNodeAssociations(NodeRef nodeRef, ExporterCrawlerParameters parameters, Exporter exporter)
        {
            ...
            for (AssociationRef assoc : assocs)
            {
                ...
                if (isExcludedNodeRef(assoc.getTargetRef())) {
                    return;
                }                
