-- Skript on mõeldud andmete obfuskeerimiseks Delta versioonil 3.6.21.
-- NB! Kui antud skripti kasutatakse hilisema kui 3.6.21 versiooni obfuskeerimiseks, ei pruugi see skript katta kõiki vajalikke andmeid. 
...
insert into public.delta_task (
	select task_id, workflow_id, task_type, index_in_workflow, wfc_status, concat(isikud_creator.eesnimi, ' ', isikud_creator.perekonnanimi),
		...
		isikud_previous_owner.isikukood, 'obf@obf.obf', wfc_owner_group, wfc_outcome, wfc_document_type, wfc_completed_date_time, 
		rpad('asutus', length(wfc_owner_organization_name), ' asut'),
		isikud_owner.amet,
		...
		wfs_confirmed_due_date, has_due_date_history, has_files, is_searchable
...
insert into public.alf_node_properties (
	...
	case when 
		...
		or (actual_type_n in (7, 11, 12, 15, 17, 18) and persisted_type_n = 6)
		then string_value
...
insert into public.alf_node_properties (
	...
	float_value, double_value, 
	case when (actual_type_n in (7, 11, 12, 15, 17, 18) and persisted_type_n = 6)
		then string_value
...
insert into tmp_content_node_ids (
	...
	where (local_name = 'folder' and uri = 'http://www.alfresco.org/model/content/1.0')
		...
		or (local_name = 'reportResult' and uri = 'http://alfresco.webmedia.ee/model/report/1.0')
		or (uri = 'http://alfresco.webmedia.ee/model/workflow/specific/1.0')
);
...
-- 8) 'confirmationTask', 'informationTask', 'assignmentTask', 'signatureTask', 'reviewTask', 'opinionTask',
-- 'externalReviewTask', 'orderAssignmentTask', 'dueDateExtensionTask' tüüpi node'idel ei obfuskeerita status välja väärtust, 
-- isikutega seotud andmed obfuskeeritakse suvalise isiku andmetega tabelist tmp_isikud,
-- ülejäänud tekstilised andmed obfuskeeritakse suvaliste väärtustega  
insert into public.alf_node (
	select node.id, node.version, node.store_id, node.uuid, node.transaction_id, node.node_deleted, node.type_qname_id, node.acl_id,
	(case when node.audit_creator = 'System' then node.audit_creator else isikud_creator.isikukood end),
	...
	left join tmp_isikud as isikud_creator on ceil(node.random1 * (select count(*) from tmp_isikud)) = isikud_creator.row_num
	left join tmp_isikud as isikud_modifier on ceil(node.random2 * (select count(*) from tmp_isikud)) = isikud_modifier.row_num
	where local_name in ('confirmationTask', 'informationTask', 'assignmentTask', 'signatureTask', 'reviewTask', 'opinionTask',
	'externalReviewTask', 'orderAssignmentTask', 'dueDateExtensionTask') and uri = 'http://alfresco.webmedia.ee/model/workflow/specific/1.0'
);
...
insert into public.alf_node_properties (
	...
	when (qname.local_name = 'creatorName' and ns.uri = 'http://alfresco.webmedia.ee/model/workflow/common/1.0')
		then concat(isikud_creator.eesnimi, ' ', isikud_creator.perekonnanimi)
	when (qname.local_name = 'creatorId' and ns.uri = 'http://alfresco.webmedia.ee/model/workflow/specific/1.0')
		then isikud_creator.isikukood	
	when (qname.local_name = 'creatorEmail' and ns.uri = 'http://alfresco.webmedia.ee/model/workflow/specific/1.0') 
		then 'obf@obf.obf'
	when (qname.local_name = 'ownerName' and ns.uri = 'http://alfresco.webmedia.ee/model/workflow/common/1.0')
		then concat(isikud_owner.eesnimi, ' ', isikud_owner.perekonnanimi)
	when (qname.local_name = 'ownerId' and ns.uri = 'http://alfresco.webmedia.ee/model/workflow/common/1.0')
		then isikud_owner.isikukood	
	when (qname.local_name = 'ownerEmail' and ns.uri = 'http://alfresco.webmedia.ee/model/workflow/common/1.0') 
		then 'obf@obf.obf'
	when (qname.local_name = 'ownerJobTitle' and ns.uri = 'http://alfresco.webmedia.ee/model/workflow/common/1.0')
		then isikud_owner.amet	
	else rpad('obfuskeeritud', length(string_value), ' obfusk') end,
	...
	qname_id, list_index, locale_id
	from 
	(select alf_node.*, random() as random1, random() as random2 from original.alf_node ) as node
	join original.alf_node_properties props on props.node_id = node.id
	join original.alf_qname type_qname on type_qname.id = node.type_qname_id
	join original.alf_namespace type_ns on type_ns.id = type_qname.ns_id
	join original.alf_qname qname on qname.id = props.qname_id
	join original.alf_namespace ns on qname.ns_id = ns.id
	left join tmp_isikud isikud_creator on ceil(node.random1 * (select count(*) from tmp_isikud)) = isikud_creator.row_num
	left join tmp_isikud isikud_owner on ceil(node.random2 * (select count(*) from tmp_isikud)) = isikud_owner.row_num
	where type_qname.local_name in ('confirmationTask', 'informationTask', 'assignmentTask', 'signatureTask', 'reviewTask', 'opinionTask',
	'externalReviewTask', 'orderAssignmentTask', 'dueDateExtensionTask') and type_ns.uri = 'http://alfresco.webmedia.ee/model/workflow/specific/1.0'
);

-- 8) dueDateHistory. Tekstilised andmed obfuskeeritakse suvaliste väärtustega  
insert into public.alf_node (
	select node.id, node.version, node.store_id, node.uuid, node.transaction_id, node.node_deleted, node.type_qname_id, node.acl_id,
	(case when node.audit_creator = 'System' then node.audit_creator else isikud_creator.isikukood end),
	node.audit_created,
	...
	left join tmp_isikud as isikud_creator on ceil(node.random1 * (select count(*) from tmp_isikud)) = isikud_creator.row_num
	left join tmp_isikud as isikud_modifier on ceil(node.random2 * (select count(*) from tmp_isikud)) = isikud_modifier.row_num
	where local_name in ('dueDateHistory') and uri = 'http://alfresco.webmedia.ee/model/workflow/common/1.0'
);
...
insert into public.alf_node_properties (
	...
	float_value, double_value, 
	case when 
		(actual_type_n in (7, 11, 12, 15, 17, 18) and persisted_type_n = 6)
		then string_value
	else rpad('obfuskeeritud', length(string_value), ' obfusk') end,
	case when serializable_value is null 
		then null
	...
	qname_id, list_index, locale_id
	from 
	(select alf_node.* from original.alf_node) as node
	join original.alf_node_properties props on props.node_id = node.id
	join original.alf_qname type_qname on type_qname.id = node.type_qname_id
	join original.alf_namespace type_ns on type_ns.id = type_qname.ns_id
	join original.alf_qname qname on qname.id = props.qname_id
	join original.alf_namespace ns on qname.ns_id = ns.id
	where type_qname.local_name in ('dueDateHistory') and type_ns.uri = 'http://alfresco.webmedia.ee/model/workflow/common/1.0'
);
...
CREATE OR REPLACE FUNCTION updateRegisterSequences() returns void AS 
$BODY$
DECLARE
    r text;
    i int;
begin
	FOR r IN select distinct relname from pg_class where relkind='S' and relname like 'register_%' LOOP
		execute ('SELECT last_value FROM original.' || r) into i;
		perform (select setval(concat('public.', r), i ));			
	END LOOP;
end
$BODY$
LANGUAGE plpgsql;
...
select updateRegisterSequences();

-- Täiendav obfuskeerimine public schemas

-- Kasutajagruppide (struktuuriüksuste) täiendav obfuskeerimine
...
-- kasutajagruppide obfuskeerimine
UPDATE alf_child_assoc SET qname_localname = 
	concat('GROUP_', case when position(',' in qname_localname) < 1 then 'obfuskeeritud' 
		else repeat('obfuskeeri, tud', array_length(regexp_split_to_array(qname_localname, '(,)'), 1)) end, id ) 
	where qname_localname like 'GROUP_%' 
	and qname_localname not in ('GROUP_ALFRESCO_ADMINISTRATORS', 'GROUP_ARCHIVISTS', 'GROUP_DOCUMENT_MANAGERS', 'GROUP_EVERYONE', 'GROUP_SUPERVISION', 'GROUP_ACCOUNTANTS');
...
set string_value = concat('fail_', props.node_id, (case when position('.' in string_value) > 0 
							then substring(string_value from (length(string_value) - position('.' in reverse(string_value)) + 1) for (length

(string_value) - position('.' in reverse(string_value)) + 6)) 
							else '' end) )
from
...
-- @politsei.ee meiliaadresse sisaldavate kirjete täiendav obfuskeerimine
...
--Dokumentide juurde salvestatud mallide nimed

UPDATE alf_node_properties set string_value =
        concat('fail_', node_id, (case when position('.' in string_value) > 0
                then substring(string_value from (length(string_value) - position('.' in reverse(string_value)) + 1) for (length(string_value) - position('.' in reverse
...
-- Aruannete täiendav obfuskeerimine (isikukoodid ja mallide nimed)

UPDATE alf_node_properties set string_value = rpad('obfuskeeritud', length(string_value), ' obfusk') 
from alf_qname qname, alf_namespace ns 
where alf_node_properties.qname_id = qname.id and ns.id = qname.ns_id and ((local_name = 'reportName' and uri = 'http://alfresco.webmedia.ee/model/report/1.0,reportResult')
or (local_name = 'userName' and uri = 'http://alfresco.webmedia.ee/model/report/1.0,reportResult'));

-- Isikukoodide täiendav obfuskeerimine! NB! oluline on seda jooksutada kõige lõpus, 
-- kui isikukoodide hägustamine teadaolevate andmete osas on juba tehtud (s.t. teadaolevad isikukoodide väljad on asendatud genereeritud isikukoodidega).
...
where qname_localname SIMILAR TO '\d\d\d\d\d\d\d\d\d\d\d' and qname_localname not like '100%';

update alf_child_assoc set child_node_name = '10000000001'
where child_node_name SIMILAR TO '\d\d\d\d\d\d\d\d\d\d\d' and child_node_name not like '100%';

update alf_node_properties set string_value = '10000000001'
where string_value SIMILAR TO '\d\d\d\d\d\d\d\d\d\d\d' and string_value not like '100%';